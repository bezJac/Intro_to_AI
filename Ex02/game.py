...
COMPUTER='#' #Marks the computer's cells on the board
HUMAN='*' #Marks the human's cells on the board
SELECTED='x'

def create():
    board=[]
    for i in range(8):
        board= board+[[generateRandom() for i in  range(8)]]
    return [board,0.00001,HUMAN,64]

def generateRandom():
    return random.randrange(-6,16)
